...
const cancelReservation = (id) => {
    ...
    if (confirm("Czy na pewno chcesz anulować tę rezerwację?")) {
        Inertia.post(`/reservations/${id}/cancel`);
    }
}
...
const canCancel = (startTime, status) => {
    ...
    const differenceInMilliseconds = reservationDate - currentDate;
    ...
    <MainContent>
        <div class="flex flex-col gap-16 items-start">
                ...
                <div>
                    ...
                    <div v-if="upcomingUsersReservations.length > 0">
                        <div class="overflow-x-auto w-full">
                            <table class="w-full whitespace-nowrap bg-primary rounded-md">
            ...
            <div v-else class="flex flex-col gap-y-12">
                <div class="grid grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-6 max-w-6xl w-full">
                    <div class="flex flex-col gap-y-3 justify-center bg-accent1 text-white font-bold text-2xl text-center p-6 rounded-xl items-center content-center">
                        Liczba rezerwacji
                        <span class="block text-secondary text-3xl w-fit p-4 bg-accent3 rounded-full">{{ reservations.length }}</span>
                    </div>
                    <div class="flex flex-col gap-y-3 justify-center bg-accent2 text-white font-bold text-2xl text-center p-6 rounded-xl items-center content-center">
                        Godziny na korcie
                        <span class="block text-center text-secondary text-3xl  w-fit p-4 bg-accent3 rounded-full">{{ totalHours }}h</span>
                    </div>
                    <div class="flex flex-col gap-y-3 bg-accent3 text-white font-bold text-2xl text-center p-6 rounded-xl">
                ...
                <div class="grid sm:grid-cols-1 max-w-6xl w-full">
                    <div class="flex justify-between items-center content-center mb-6">
                        <h2 class="text-3xl font-bold">Ostatnie rezerwacje</h2>
                        <Link href="/my-reservations" class="bg-accent1 text-white font-bold w-fit py-2 px-4 rounded-lg hover:underline">Zobacz wszystkie</Link>
                    </div>
                    <div v-if="reservations.length > 0">
                        <div class="overflow-x-auto w-full">
                            <table class="w-full whitespace-nowrap bg-primary rounded-md">
                                ...
                                <tr v-for="reservation in reservations.slice(0, 5)"
                                    :class="{'grass': reservation.court.surface === 'grass', 'clay': reservation.court.surface === 'clay', 'hard': reservation.court.surface === 'hard'}"
                                    class="p-4 m-4 hover:bg-accent1">
                                    <td>
                                        <div class="flex items-center px-6 py-4 focus:text-indigo-500" :href="`/reservations/${reservation.id}/edit`">
                                    ...
                                    <td>
                                        <DangerButton class="mr-6" v-if="canCancel(reservation.start_time, reservation.status)" @click="cancelReservation(reservation.id)">
                                            Rezygnuj
                                        </DangerButton>
                                    </td>
                                </tr>
